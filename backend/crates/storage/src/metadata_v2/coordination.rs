...
impl MetadataCoordination for ObjectStoreCoordination {
    async fn acquire_lease(
        ...
        if let Some((existing, _)) =
            self.doc_store
                .get::<LeaseDocument>(&key)
                .await
                .map_err(|e| {
                    CoordinationError::Other(anyhow::anyhow!(
                        "Failed to check existing lease: {}",
                        e
                    ))
                })?
        {
    ...
    async fn release_lease(&self, lease: &Lease) -> Result<(), CoordinationError> {
        ...
        if let Some((existing, _metadata)) = self
            .doc_store
            .get::<LeaseDocument>(&key)
            .await
            .map_err(|e| CoordinationError::Other(anyhow::anyhow!("Failed to get lease: {}", e)))?
        {
            if existing.token != lease.token {
    ...
    async fn extend_lease(
        ...
        if let Some((existing, metadata)) = self
            .doc_store
            .get::<LeaseDocument>(&key)
            .await
            .map_err(|e| CoordinationError::Other(anyhow::anyhow!("Failed to get lease: {}", e)))?
...
mod tests {
    ...
    async fn test_in_memory_coordination() {
        ...
        // Acquire lease
        let lease = coord
            .acquire_lease("resource1", 60, "test-owner")
            .await
            .unwrap();
