...
use crate::metadata_v2::{
    schemas::UserDocument, MetadataDocumentStore, MetadataDocumentStoreExt, PutOptions,
};
...
impl RustFsUserRepository {
    ...
    async fn update_indexes_for_create(
        &self,
        user: &UserDocument,
...
impl UserRepository for RustFsUserRepository {
    ...
    async fn list_users(
        ...
        let user_ids: Vec<Uuid> = list.user_ids.into_iter().skip(offset).take(limit).collect();
...
mod tests {
    ...
    async fn create_test_repository() -> (RustFsUserRepository, TempDir) {
        ...
        let repo =
            RustFsUserRepository::new(doc_store, "apps/rustshare".to_string(), "test".to_string());
