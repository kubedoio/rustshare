...
pub trait FolderRepository: Send + Sync {
    ...
    async fn get_required(&self, id: FolderId) -> Result<FolderDocument, RepositoryError> {
        self.get(id)
            .await?
            .ok_or_else(|| RepositoryError::NotFound(format!("Folder not found: {}", id)))
    }
    ...
    async fn get_user_roots(&self, user_id: UserId)
        -> Result<Vec<FolderDocument>, RepositoryError>;
...
pub trait FileRepository: Send + Sync {
    ...
    async fn get_required(&self, id: FileId) -> Result<FileDocument, RepositoryError> {
        self.get(id)
            .await?
            .ok_or_else(|| RepositoryError::NotFound(format!("File not found: {}", id)))
    }
...
pub trait ShareRepository: Send + Sync {
    ...
    async fn get_required(&self, id: ShareId) -> Result<ShareDocument, RepositoryError> {
        self.get(id)
            .await?
            .ok_or_else(|| RepositoryError::NotFound(format!("Share not found: {}", id)))
    }
    ...
    async fn list_by_creator(&self, user_id: UserId)
        -> Result<Vec<ShareDocument>, RepositoryError>;
