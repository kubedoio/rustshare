...
impl RustFsNotificationRepository {
    ...
    async fn get_or_create_index(
        &self,
        user_id: Uuid,
    ) -> Result<UserNotificationIndex, NotificationRepositoryError> {
        let index_path = self.path_builder.user_index_path(user_id);

        match self
            .doc_store
            .get::<UserNotificationIndex>(&index_path)
            .await
        {
...
impl NotificationRepository for RustFsNotificationRepository {
    ...
    async fn get_user_notifications(
        ...
        let paths: Vec<String> = refs
            ...
            .map(|r| {
                self.path_builder
                    .notification_path(user_id, r.notification_id)
            })
            .collect();
        ...
        if !path_refs.is_empty() {
            match self
                .doc_store
                .get_multi::<NotificationDocument>(&path_refs)
                .await
            {
                Ok(results) => {
        ...
        if count > 0 {
            ...
            for notif_ref in &mut index.notifications {
                if !notif_ref.read {
                    ...
                    let path = self
                        .path_builder
                        .notification_path(user_id, notif_ref.notification_id);
                    if let Ok(Some((mut doc, meta))) =
                        self.doc_store.get::<NotificationDocument>(&path).await
                    {
                        doc.read = true;
                        doc.read_at = Some(chrono::Utc::now());
                        if let Err(e) = self
...
mod tests {
    ...
    async fn test_get_notification() {
        ...
        let missing = repo
            .get_notification(user_id, Uuid::new_v4())
            .await
            .unwrap();
    ...
    async fn test_mark_read() {
        ...
        let retrieved = repo
            .get_notification(user_id, notif_id)
            .await
            .unwrap()
            .unwrap();
