...
impl<R: JobRepository, C: CoordinationStore> JobCoordinator<R, C> {
    ...
    pub fn new(job_repo: Arc<R>, coord_store: Arc<C>, worker_id: String) -> Self {
        Self {
            job_repo,
            coord_store,
            worker_id,
    ...
    pub async fn claim_job(&self) -> Result<Option<Job>, CoordinatorError> {
        ...
        for job in pending {
            ...
            match self
                .coord_store
                .claim_job(&job.id.to_string(), &self.worker_id, self.lease_duration)
                .await
            {
                Ok(true) => {
                    ...
                    if let Err(e) = self.job_repo.update_job(&job).await {
                        ...
                        if let Err(release_err) = self
                            .coord_store
                            .release_job_claim(&job.id.to_string(), &self.worker_id)
                            .await
                        {
    ...
    pub async fn heartbeat(&self, job_id: Uuid) -> Result<bool, CoordinatorError> {
        match self
            .coord_store
            .heartbeat_job(&job_id.to_string(), &self.worker_id, self.lease_duration)
            .await
        {
            Ok(true) => {
    ...
    pub async fn fail_job(
        ...
        if job.retry_count >= job.max_retries {
            ...
            job.started_at = None;
            job.scheduled_at =
                chrono::Utc::now() + chrono::Duration::seconds(60 * job.retry_count as i64);
            warn!(
                "Job {} failed (attempt {}/{}), rescheduled",
...
mod tests {
    ...
    async fn create_test_coordinator() -> (
        JobCoordinator<RustFsJobRepository, InMemoryCoordinationStore>,
        TempDir,
    ) {
        ...
        let coord_store: Arc<InMemoryCoordinationStore> =
            Arc::new(InMemoryCoordinationStore::new());

        let coordinator = JobCoordinator::new(job_repo, coord_store, "worker1".to_string());
    ...
    async fn test_complete_job() {
        ...
        // Complete it
        coordinator
            .complete_job(claimed.id, serde_json::json!({"success": true}))
            .await
            .unwrap();
    ...
    async fn test_fail_job_with_retry() {
        ...
        // Fail it
        coordinator
            .fail_job(claimed.id, "Network error".to_string())
            .await
            .unwrap();
    ...
    async fn test_fail_job_permanently() {
        ...
        coordinator
            .fail_job(claimed.id, "Final error".to_string())
            .await
            .unwrap();
