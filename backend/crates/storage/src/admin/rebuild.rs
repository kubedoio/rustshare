...
impl RebuildTool {
    ...
    pub async fn rebuild_folder_children_index(
        &self,
        folder_id: Uuid,
    ...
    pub async fn rebuild_user_folder_indexes(
        &self,
        user_id: Uuid,
    ) -> Result<OperationSummary, RepositoryError> {
            ...
            match self.rebuild_folder_children_index_recursive(root.id).await {
                ...
                Err(e) => {
                    summary.add_error(format!(
                        "Failed to rebuild index for folder {}: {}",
                        root.id, e
                    ));
                }
            }
    ...
    pub async fn rebuild_user_roots_index(
        &self,
        user_id: Uuid,
    ) -> Result<OperationSummary, RepositoryError> {
    ...
    pub async fn rebuild_shared_with_me_index(
        &self,
        user_id: Uuid,
    ) -> Result<OperationSummary, RepositoryError> {
        ...
        for share in shares {
            ...
            let resource_name = match share.resource_type.as_str() {
                "file" => match self.repo.files().get(share.resource_id).await? {
                    Some(file) => file.name,
                    None => {
                ...
                "folder" => match self.repo.folders().get(share.resource_id).await? {
