...
impl VerificationReport {
    ...
    pub fn overall_pass_rate(&self) -> f64 {
        let total = self.folders_verified + self.files_verified + self.shares_verified + self.versions_verified + self.indexes_verified;
        let passed = self.folders_passed + self.files_passed + self.shares_passed + self.versions_passed + self.indexes_passed;
...
impl ParityVerifier {
    ...
    pub async fn verify_folder(&self, folder_id: Uuid) -> Result<VerificationResult, RepositoryError> {
        ...
        match (pg_folder, rustfs_folder) {
            (Some(pg), Some(rustfs)) => {
                ...
                if pg.parent_id != rustfs.parent_id {
                    result.add_error(format!("Parent mismatch: {:?} vs {:?}", pg.parent_id, rustfs.parent_id));
                }
                if pg.owner_id != rustfs.owner_id {
                    result.add_error(format!("Owner mismatch: {} vs {}", pg.owner_id, rustfs.owner_id));
                }
                if pg.version != rustfs.version {
                    result.add_error(format!("Version mismatch: {} vs {}", pg.version, rustfs.version));
                }
                if pg.deleted != rustfs.deleted {
                    result.add_error(format!("Deleted flag mismatch: {} vs {}", pg.deleted, rustfs.deleted));
                }
            }
            (Some(_), None) => {
        ...
        match (pg_file, rustfs_file) {
            (Some(pg), Some(rustfs)) => {
                ...
                if pg.content_ref != rustfs.content_ref {
                    result.add_error(format!("Content ref mismatch: {} vs {}", pg.content_ref, rustfs.content_ref));
                }
    ...
    pub async fn verify_share(&self, share_id: Uuid) -> Result<VerificationResult, RepositoryError> {
        ...
        match (pg_share, rustfs_share) {
            (Some(pg), Some(rustfs)) => {
                ...
                if pg.resource_type != rustfs.resource_type {
                    result.add_error(format!("Resource type mismatch: {} vs {}", pg.resource_type, rustfs.resource_type));
                }
                if pg.resource_id != rustfs.resource_id {
                    result.add_error(format!("Resource ID mismatch: {} vs {}", pg.resource_id, rustfs.resource_id));
                }
                if pg.permissions != rustfs.permissions {
                    result.add_error(format!("Permissions mismatch: {:?} vs {:?}", pg.permissions, rustfs.permissions));
                }
                if pg.revoked_at != rustfs.revoked_at {
                    result.add_error(format!("Revoked_at mismatch: {:?} vs {:?}", pg.revoked_at, rustfs.revoked_at));
                }
            }
            (Some(_), None) => {
