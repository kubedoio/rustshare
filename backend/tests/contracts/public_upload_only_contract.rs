...
async fn test_anonymous_can_upload_via_upload_only_link() {
    ...
    let session = share_service
        .validate_and_create_session(&token, None, Some(tenant_id))
        .await
...
async fn test_anonymous_cannot_list_files_via_upload_only() {
    ...
    let token = share.share_token.unwrap();
    let public_info = share_service
        .get_public_share_info(&token, Some(tenant_id))
        .await;
...
async fn test_upload_only_with_password_requires_password() {
    ...
    let result = share_service
        .validate_and_create_session(&token, Some("wrong_password".to_string()), Some(tenant_id))
        .await;
    assert!(
    ...
    let session = share_service
        .validate_and_create_session(&token, Some("upload_password".to_string()), Some(tenant_id))
        .await;
...
async fn test_upload_only_session_cannot_list_folder_contents() {
    ...
    let result = share_service
        .list_public_folder_contents(&token, None, Some(tenant_id))
        .await;
    assert!(
        result.is_err(),
