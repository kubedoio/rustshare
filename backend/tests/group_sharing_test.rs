...
async fn test_compat_layer_find_user_by_id() {
    ...
    assert!(found_user.is_some(), "Should find the user by ID via compat layer");
    ...
    assert!(not_found.is_none(), "Should not find a non-existent user via compat layer");
...
async fn create_test_compat(pool: sqlx::PgPool) -> MetadataStoreCompat {
    use rustshare_storage::repos::{
        FolderRepository, FileRepository, FileVersionRepository, ShareRepository,
        EventRepository, FolderChildrenIndexRepository, TombstoneRepository, SearchIndexRepository,
        RepositoryError
    };
    ...
    impl FolderRepository for MockFolderRepo {
        ...
        async fn list_descendants(&self, _folder_id: FolderId) -> Result<Vec<FolderDocument>, RepositoryError> {
            Ok(vec![])
        }
        async fn get_user_roots(&self, _user_id: UserId) -> Result<Vec<FolderDocument>, RepositoryError> {
            Ok(vec![])
        }
        async fn name_exists(&self, _parent_id: Option<FolderId>, _name: &str, _owner_id: UserId) -> Result<bool, RepositoryError> {
            Ok(false)
        }
    }
    ...
    impl FileVersionRepository for MockFileVersionRepo {
        async fn get(&self, _version_id: uuid::Uuid) -> Result<Option<FileVersionDocument>, RepositoryError> {
            Ok(None)
        }
        async fn get_by_number(&self, _file_id: FileId, _version_number: i32) -> Result<Option<FileVersionDocument>, RepositoryError> {
            Ok(None)
        }
        async fn create(&self, _version: &FileVersionDocument) -> Result<(), RepositoryError> {
            Ok(())
        }
        async fn list_by_file(&self, _file_id: FileId) -> Result<Vec<FileVersionDocument>, RepositoryError> {
            Ok(vec![])
        }
        async fn get_latest(&self, _file_id: FileId) -> Result<Option<FileVersionDocument>, RepositoryError> {
            Ok(None)
        }
    }
    ...
    impl ShareRepository for MockShareRepo {
        ...
        async fn get_by_token(&self, _token_hash: &str) -> Result<Option<ShareDocument>, RepositoryError> {
            Ok(None)
        }
        ...
        async fn list_by_resource(&self, _resource_type: &str, _resource_id: uuid::Uuid) -> Result<Vec<ShareDocument>, RepositoryError> {
            Ok(vec![])
        }
        async fn list_by_creator(&self, _user_id: UserId) -> Result<Vec<ShareDocument>, RepositoryError> {
            Ok(vec![])
        }
        async fn list_by_recipient(&self, _user_id: UserId) -> Result<Vec<ShareDocument>, RepositoryError> {
            Ok(vec![])
        }
    }
    ...
    impl EventRepository for MockEventRepo {
        ...
        async fn read_for_resource(&self, _resource_type: &str, _resource_id: uuid::Uuid, _limit: usize) -> Result<Vec<EventDocument>, RepositoryError> {
            Ok(vec![])
        }
        async fn read_range(&self, _start: chrono::DateTime<chrono::Utc>, _end: chrono::DateTime<chrono::Utc>, _limit: usize) -> Result<Vec<EventDocument>, RepositoryError> {
            Ok(vec![])
        }
    }
    ...
    impl FolderChildrenIndexRepository for MockFolderChildrenIndexRepo {
        async fn get(&self, _folder_id: FolderId) -> Result<Option<FolderChildrenIndex>, RepositoryError> {
            Ok(None)
        }
        async fn save(&self, _index: &FolderChildrenIndex) -> Result<(), RepositoryError> {
            Ok(())
        }
        async fn rebuild(&self, _folder_id: FolderId) -> Result<FolderChildrenIndex, RepositoryError> {
    ...
    impl TombstoneRepository for MockTombstoneRepo {
        async fn get(&self, _resource_type: &str, _resource_id: uuid::Uuid) -> Result<Option<TombstoneDocument>, RepositoryError> {
            Ok(None)
        }
        async fn create(&self, _tombstone: &TombstoneDocument) -> Result<(), RepositoryError> {
            Ok(())
        }
        async fn list_by_user(&self, _user_id: UserId) -> Result<Vec<TombstoneDocument>, RepositoryError> {
            Ok(vec![])
        }
        async fn delete(&self, _resource_type: &str, _resource_id: uuid::Uuid) -> Result<(), RepositoryError> {
            Ok(())
        }
    }
    ...
    impl SearchIndexRepository for MockSearchIndexRepo {
        ...
        async fn search(&self, _query: &str, _tenant_id: uuid::Uuid, _user_id: UserId, _limit: usize) -> Result<Vec<uuid::Uuid>, RepositoryError> {
            Ok(vec![])
        }
    }
