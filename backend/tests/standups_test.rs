//! Contract tests for RustShare Standup Records module.
//!
//! Run with: cargo test --test standups_test -- --ignored
...
async fn contract_create_standup_creates_folder_structure_and_metadata() {
    let (pool, event_store, metadata_store, object_store) = setup_test_env().await;
    let tenant_id = Uuid::new_v4();
    let user = create_test_user(&metadata_store, "standup_contract_user_1", tenant_id).await;
    let service = create_standup_service(event_store, metadata_store.clone(), object_store, &pool);
    ...
    let standup = service
        .create_standup(
            ...
            chrono::Utc::now(),
            "- Alice: working on auth\n- Bob: fixing UI".to_string(),
        )
        .await
        .expect("create_standup should succeed");

    assert!(
        standup.path.starts_with("/Workspace/Standups/"),
        "Standup should be in canonical /Workspace/Standups folder, got path: {}",
        standup.path
    );
    assert_eq!(standup.metadata.title, "Daily Standup");
    assert_eq!(standup.metadata.kind, "standup");
...
#[tokio::test]
#[ignore = "Requires database and S3"]
async fn contract_get_standup_returns_content_and_metadata() {
    let (pool, event_store, metadata_store, object_store) = setup_test_env().await;
    let tenant_id = Uuid::new_v4();
    let user = create_test_user(&metadata_store, "standup_contract_user_2", tenant_id).await;
    let service = create_standup_service(event_store, metadata_store.clone(), object_store, &pool);
...
#[tokio::test]
#[ignore = "Requires database and S3"]
async fn contract_update_standup_updates_content_and_metadata() {
    let (pool, event_store, metadata_store, object_store) = setup_test_env().await;
    let tenant_id = Uuid::new_v4();
    let user = create_test_user(&metadata_store, "standup_contract_user_3", tenant_id).await;
    let service = create_standup_service(event_store, metadata_store.clone(), object_store, &pool);
...
#[tokio::test]
#[ignore = "Requires database and S3"]
async fn contract_cross_tenant_get_standup_denied() {
    let (pool, event_store, metadata_store, object_store) = setup_test_env().await;
    let tenant_a = Uuid::new_v4();
    let tenant_b = Uuid::new_v4();
    let user_a = create_test_user(&metadata_store, "standup_user_a", tenant_a).await;
    let user_b = create_test_user(&metadata_store, "standup_user_b", tenant_b).await;
    let service = create_standup_service(event_store, metadata_store.clone(), object_store, &pool);
...
#[tokio::test]
#[ignore = "Requires database and S3"]
async fn contract_cross_tenant_update_standup_denied() {
    let (pool, event_store, metadata_store, object_store) = setup_test_env().await;
    let tenant_a = Uuid::new_v4();
    let tenant_b = Uuid::new_v4();
    let user_a = create_test_user(&metadata_store, "standup_user_a2", tenant_a).await;
    let user_b = create_test_user(&metadata_store, "standup_user_b2", tenant_b).await;
    let service = create_standup_service(event_store, metadata_store.clone(), object_store, &pool);
...
#[tokio::test]
#[ignore = "Requires database and S3"]
async fn contract_cross_tenant_list_standups_does_not_leak() {
    let (pool, event_store, metadata_store, object_store) = setup_test_env().await;
    let tenant_a = Uuid::new_v4();
    let tenant_b = Uuid::new_v4();
    let user_a = create_test_user(&metadata_store, "standup_user_a3", tenant_a).await;
    let user_b = create_test_user(&metadata_store, "standup_user_b3", tenant_b).await;
    let service = create_standup_service(event_store, metadata_store.clone(), object_store, &pool);
...
#[tokio::test]
#[ignore = "Requires database and S3"]
async fn contract_same_tenant_unauthorized_get_standup_denied() {
    let (pool, event_store, metadata_store, object_store) = setup_test_env().await;
    let tenant_id = Uuid::new_v4();
    let user_owner = create_test_user(&metadata_store, "standup_owner", tenant_id).await;
    let user_other = create_test_user(&metadata_store, "standup_other", tenant_id).await;
    let service = create_standup_service(event_store, metadata_store.clone(), object_store, &pool);
...
#[tokio::test]
#[ignore = "Requires database and S3"]
async fn contract_same_tenant_unauthorized_update_standup_denied() {
    let (pool, event_store, metadata_store, object_store) = setup_test_env().await;
    let tenant_id = Uuid::new_v4();
    let user_owner = create_test_user(&metadata_store, "standup_owner_update", tenant_id).await;
    let user_other = create_test_user(&metadata_store, "standup_other_update", tenant_id).await;
    let service = create_standup_service(event_store, metadata_store.clone(), object_store, &pool);
    ...
    assert!(
        matches!(result, Err(StandupError::PermissionDenied)),
        "Same-tenant unauthorized update_standup should be denied, got {:?}",
        result
    );
