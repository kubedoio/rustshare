...
# Buzz identity consistency is validated by the containerized Chat bootstrap.
# Keeping this pre-flight shell-only is important: a clean operator must not
# install host Node/npm just to initialize the managed Chat runtime.
...
echo "    scripts/read-bootstrap-password.sh \"\$(docker compose ps -q backend)\""
