...
# stream message scoped by ["h", <channel>] when BUZZ_CHANNEL_ID is set, else
# the legacy kind-1 note) and publishes it over NIP-42 against an
# operator-supplied disposable relay (ADR-0034 recipe), plus a MANUAL
# revocation checklist — the 9030/9031 relay orchestration needs the relay's
# own admin CLI, so revocation is not automated here (the Elembra-side
...
#                      (default: legacy kind-1 note, for relays with no channel)
...
echo "== 2. publish a signed event (kind 9 with BUZZ_CHANNEL_ID, else kind 1) =="
