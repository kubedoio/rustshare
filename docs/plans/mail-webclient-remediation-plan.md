...
| done | M-P2-17 duplicate send prevention | `backend/server/src/handlers/mail.rs`; `backend/server/src/services/mail_service.rs`; `backend/migrations/20260716000000_mail_attachment_content_id.sql`; `frontend/src/lib/components/modules/MailComposeModal.svelte` | SMTP integration test retries one idempotency key and expects one server delivery; draft send uses a per-draft advisory lock | Retries and double-clicks do not duplicate outbound mail |
| deferred | M-P2-11 blob lifecycle | Shared object-store/file-version ownership requires a repository-wide reference-aware GC design | Existing content-addressed keys can be shared by mail, promoted files, and file versions; naive mail deletion would destroy live data | Blocked pending reference-aware blob lifecycle design and migration |
...
| deferred | M-P2-24 decorative selected-import test | `backend/tests/mail_imap_import_test.rs`; `backend/server/src/services/mail_service.rs`; `backend/server/src/services/imap_client.rs` | Requires a mockable selected-import session/storage boundary; current public-host test remains ignored | Blocked test architecture; production UIDVALIDITY/dedup/reclaim logic is unchanged |
