...
| deferred | M-P2-11 blob lifecycle | `backend/migrations/20260717000000_reference_aware_blob_gc.sql`; `backend/crates/storage/src/metadata.rs`; `backend/server/src/retention.rs`; `backend/server/src/bootstrap.rs` | Corrective migration queues removed keys; retention rechecks references before deleting unique objects and retains shared `blobs/<sha256>` keys until writers and GC share a cross-process lease | Unique objects can be reclaimed safely; content-addressed blob reclamation remains disabled to prevent a concurrent writer/GC data-loss race |
...
| done | M-P2-10 raw UUID link UI | `frontend/src/routes/(app)/modules/mail/messages/[messageId]/+page.svelte` | Existing file API supplies selectable names; other backend-supported artifact types retain identifier entry | File linking uses a discoverable picker and displays file names without removing existing non-file link workflows |
