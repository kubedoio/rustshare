...
- **Body gate at observation.** `BuzzObservationService` stores `chat_observed_events.body` only when `content_indexing` is on at observation time AND the event's channel is a `workspace` channel. With the flag off — or for a `dm`/`private`/`excluded` channel — observation rows carry no body: bodies from never-eligible channels are never captured, so they cannot leak into an indexing copy under the tenant's opt-in.
- **No backfill (documented limitation).** The observation `body` column is captured at observation time only. It is NOT refreshed by re-push (`DuplicateObservation` is a no-op) or by reconciliation; enabling `content_indexing` after the fact does not backfill bodies, and restoring a corrupted/lost body column requires deleting the affected observation rows and re-pushing from Buzz. This is consistent with the reference-first model: bodies are opt-in indexing copies, never authoritative.
