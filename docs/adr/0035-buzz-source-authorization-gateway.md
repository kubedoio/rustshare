...
- [x] Real relay endpoints implemented in the Buzz repository (proposed spec).
  **Satisfied:** kubedoio/buzz PR #1 (merged; the supported implementation is
  pinned in `config/buzz-compatibility.env`) implements `POST /api/v1/relay/access/check`,
...
- [x] Live-relay conformance test replacing the fake before production buzz mode.
  **Satisfied:** `backend/tests/buzz_live_conformance_test.rs` (13 live proofs:
  P1–P7 and P9–P14,
  run by `scripts/run-buzz-conformance.sh` against the real pinned relay image)
  replaces the fake relay for the production-authority
