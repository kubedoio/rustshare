...
Status: Implemented v1alpha1 (commit pending on `codex/buzz-source-authorization`)  
Date: 2026-08-11  
...
- `backend/tests/buzz_authority_gateway_test.rs` — the 14-requirement + 4-negative acceptance
  suite (see §6 and the design spec §6) against an in-test fake Buzz relay.
...
- **`wss://` enforcement is operational.** Production MUST use `wss://` relay URLs (so the
  derived base is `https://`); plaintext is acceptable only for local development/testing — the
  signature pinning, echo, and freshness checks defeat MITM replay but not observation.
