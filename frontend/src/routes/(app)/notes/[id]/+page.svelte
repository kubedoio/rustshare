...
<div class="note-editor-page">
  ...
  <header class="note-header">
    ...
    <div class="note-header-right">
      ...
      {#if !isLoading && note}
        ...
        <button class="btn btn-ghost btn-sm text-error" on:click={() => showDeleteConfirm = true}>
          <Trash2 size={14} />
        </button>
      {/if}
    </div>
  ...
  {#if note?.metadata.visibility === 'public' && note.metadata.public_share_id}
    <div class="public-url-banner">
      <div class="public-url-content">
        ...
        <button class="btn btn-xs btn-ghost" on:click={copyPublicUrl}>
  ...
  {#if saveError}
    <div class="error-banner">
      <AlertCircle size={16} />
      <span>{saveError}</span>
      <button class="btn btn-xs btn-ghost" on:click={() => saveError = null}>Dismiss</button>
    </div>
  {/if}
...
{#if showDeleteConfirm}
  <div class="modal modal-open">
    <div class="modal-box">
      ...
      <p class="py-4">This will move "{title || 'Untitled Note'}" to trash. You can restore it later.</p>
