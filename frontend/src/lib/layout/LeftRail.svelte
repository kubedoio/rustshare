	...
	<nav class="flex-1 space-y-1 px-2 py-4" aria-label="Primary">
		{#each primaryItems as item}
			...
			>
				...
				<!-- Tooltip -->
				<span class="invisible absolute left-full z-50 ml-3 whitespace-nowrap rounded-lg border border-base-300/70 bg-base-100 px-2.5 py-1.5 text-xs font-medium text-base-content opacity-0 shadow-lg transition-all duration-200 group-hover:visible group-hover:opacity-100">
					{item.label}
				</span>
			</a>
		{/each}
