...
function handleFileUploaded(event: WebSocketEvent): void {
  ...
  // Invalidate folder contents where file was uploaded
  queryClient.invalidateQueries({ queryKey: ['folder-contents', payload.folder_id] });

  // Also invalidate root if folder_id is null
  if (!payload.folder_id) {
    queryClient.invalidateQueries({ queryKey: ['folder-contents', null] });
  }

  if (!isOwnEvent(event)) {
    toastStore.show(`File "${payload.file_name}" was uploaded`, 'info');
  }
}
...
function handleFileModified(event: WebSocketEvent): void {
  ...
  if (!isOwnEvent(event)) {
    toastStore.show(`File "${payload.file_name}" was modified (v${payload.version})`, 'info');
  }
}
...
function handleFileRenamed(event: WebSocketEvent): void {
  ...
  if (!isOwnEvent(event)) {
    toastStore.show(`File renamed from "${payload.old_name}" to "${payload.new_name}"`, 'info');
  }
}
...
function handleFileMoved(event: WebSocketEvent): void {
  ...
  queryClient.invalidateQueries({ queryKey: ['folder-contents', payload.old_folder_id] });
  queryClient.invalidateQueries({ queryKey: ['folder-contents', payload.new_folder_id] });
  queryClient.invalidateQueries({ queryKey: ['file', payload.file_id] });
...
function handleFolderCreated(event: WebSocketEvent): void {
  ...
  queryClient.invalidateQueries({ queryKey: ['folder-contents', payload.parent_folder_id] });
...
function handleFolderRenamed(event: WebSocketEvent): void {
  ...
  if (!isOwnEvent(event)) {
    toastStore.show(`Folder renamed from "${payload.old_name}" to "${payload.new_name}"`, 'info');
  }
}
...
function handleFolderMoved(event: WebSocketEvent): void {
  ...
  queryClient.invalidateQueries({ queryKey: ['folder-contents', payload.old_parent_id] });
  queryClient.invalidateQueries({ queryKey: ['folder-contents', payload.new_parent_id] });
  queryClient.invalidateQueries({ queryKey: ['folders'] });
