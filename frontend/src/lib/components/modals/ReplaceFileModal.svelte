		...
		try {
			...
			if (err instanceof Error) {
				if (err.message.includes('409')) {
					error =
						'Version conflict: The file was modified by someone else. Please refresh and try again.';
				} else {
					error = err.message;
				}
			} else {
...
<ModalBase {open} title="Replace File" onClose={handleClose}>
	{#if file}
		<p class="mb-4">
		...
		<div class="modal-action">
			<button type="button" class="btn" onclick={handleClose} disabled={uploading}> Cancel </button>
			<button
				type="button"
