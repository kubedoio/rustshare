				...
				{:else if file.name.toLowerCase().endsWith('.md') && textContent !== null}
					<div class="h-full w-full overflow-auto bg-base-100 p-8">
						<article class="prose max-w-none">
							<!-- Stored-XSS guard: file contents are attacker-controlled, so the
							     rendered Markdown must pass through the DOMPurify sanitizer. -->
							{@html sanitizeHtml(markdownToHtml(textContent).html)}
						</article>
					</div>
