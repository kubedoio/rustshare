	...
	const restoreMutation = createMutation({
		...
		onError: (error) => {
			if (error instanceof ApiError && error.status === 409) {
				conflictError = true;
				errorMessage =
					'The file has been modified since you viewed the version history. Please reload and try again.';
			} else {
...
{#if open && fileId}
	<div class="modal-open modal">
		<div class="modal-box max-w-3xl">
			...
			{:else}
				<div class="py-8 text-center text-base-content/60">No version history available</div>
			{/if}
	...
	<div class="modal-open modal">
		<div class="modal-box">
			...
			{#if conflictError}
				...
				<div class="modal-action">
					<button type="button" class="btn btn-ghost" onclick={cancelRestore}> Cancel </button>
					<button type="button" class="btn btn-primary" onclick={reloadVersions}> Reload </button>
				</div>
			{:else}
				<p class="mb-4">
					Are you sure you want to restore <strong>{fileName}</strong> to version
