...
import BrainstormingApplicationView from './BrainstormingApplicationView.svelte';
...
describe('BrainstormingApplicationView', () => {
	beforeEach(() => {
		vi.clearAllMocks();
		queryClient.clear();
	});
	...
	it('renders empty state when no boards exist', async () => {
		...
		render(BrainstormingApplicationView, { module: mockModule });
	...
	it('renders board gallery with thumbnails', async () => {
		...
		expect(link.getAttribute('href')).toBe('/apps/brainstorming/board-1');
	});
