...
import MailApplicationView from './MailApplicationView.svelte';
...
const testModule = {
	...
	page: { enabled: true, route: '/apps/mail', renderer: 'mail-list', layout: 'list-grid' },
...
describe('MailApplicationView', () => {
	...
	it('loads and sanitizes the selected remote body', async () => {
		const { container } = render(MailApplicationView, { module: testModule });
	...
	it('navigates from Saved to RustShare to the imported message', async () => {
		...
		expect(mocks.goto).toHaveBeenCalledWith('/apps/mail/messages/saved-1');
	});
