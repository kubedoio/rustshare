...
`;
	...
	async function handleRenameConfirm(newTitle: string) {
		...
		renameError = '';
		try {
			await standupsApi.rename(activeItem.id, { title: trimmed });
			toastStore.show('Standup renamed', 'success');
			showRenameModal = false;
			$standupsQuery.refetch();
		} catch (err) {
			console.error('Failed to rename standup:', err);
	...
	async function handleMoveConfirm(payload: { targetFolderId: string | null }) {
		...
		isMoving = true;
		try {
			await standupsApi.move(activeItem.id, { target_folder_id: payload.targetFolderId });
			toastStore.show('Standup moved', 'success');
			showMoveModal = false;
			$standupsQuery.refetch();
		} catch (err) {
			console.error('Failed to move standup:', err);
	...
	async function handleDuplicate(item: any) {
		...
		isDuplicating = true;
		try {
			const duplicated = await standupsApi.duplicate(item.id);
			toastStore.show('Standup duplicated', 'success');
			goto(`/modules/${module.key}/${duplicated.id}`);
			$standupsQuery.refetch();
		} catch (err) {
			console.error('Failed to duplicate standup:', err);
			toastStore.show(err instanceof Error ? err.message : 'Failed to duplicate standup', 'error');
		} finally {
			isDuplicating = false;
		}
	}
	...
	async function handleDeleteConfirm() {
		...
		isDeleting = true;
		try {
			await standupsApi.delete(activeItem.id);
			toastStore.show('Standup deleted', 'success');
				...
				{#if viewMode === 'grid'}
					<div class="grid gap-3 p-3 sm:grid-cols-2 xl:grid-cols-3">
						{#each visibleStandups as standup}
							...
							>
								<a href={`/modules/${module.key}/${standup.id}`} class="block">
									<div
								...
								{@render itemActions(standup, 'grid')}
							</div>
						{/each}
					</div>
				{:else}
					<div class="divide-y divide-base-200">
						{#each visibleStandups as standup}
							<div class="flex items-center gap-4 px-4 py-3 transition-colors hover:bg-base-200/40">
								...
								{@render itemActions(standup, 'list')}
							</div>
						{/each}
					</div>
				{/if}
