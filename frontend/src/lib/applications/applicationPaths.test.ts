...
import {
	getApplicationRoot,
	getLegacyApplicationRoot,
	isWorkspacePath,
	isLegacyApplicationRoot,
	getApplicationPathVariants,
	getCanonicalWritePath,
	getApplicationReadPaths,
	resolvePathInTree,
	LEGACY_MODULE_ROOTS
} from './applicationPaths';
...
describe('applicationPaths', () => {
	describe('getApplicationRoot', () => {
		it('returns Workspace-prefixed path for Notes', () => {
			expect(getApplicationRoot('Notes')).toBe('/Workspace/Notes');
		});

		it('returns Workspace-prefixed path for Kanban', () => {
			expect(getApplicationRoot('Kanban')).toBe('/Workspace/Kanban');
		});

		it('returns Workspace-prefixed path for any module', () => {
			expect(getApplicationRoot('Meetings')).toBe('/Workspace/Meetings');
			expect(getApplicationRoot('Standups')).toBe('/Workspace/Standups');
			expect(getApplicationRoot('Decisions')).toBe('/Workspace/Decisions');
			expect(getApplicationRoot('Brainstorming')).toBe('/Workspace/Brainstorming');
			expect(getApplicationRoot('Shares')).toBe('/Workspace/Shares');
		});
	});

	describe('getLegacyApplicationRoot', () => {
		it('returns direct root path', () => {
			expect(getLegacyApplicationRoot('Notes')).toBe('/Notes');
			expect(getLegacyApplicationRoot('Kanban')).toBe('/Kanban');
		});
	});
	...
	describe('getApplicationReadPaths', () => {
		it('includes both canonical and legacy paths', () => {
			expect(getApplicationReadPaths('Notes')).toEqual(['/Workspace/Notes', '/Notes']);
		});

		it('lists canonical path first', () => {
			const paths = getApplicationReadPaths('Standups');
	...
	describe('isLegacyApplicationRoot', () => {
		it('returns true for known legacy module roots', () => {
			expect(isLegacyApplicationRoot('/Notes')).toBe(true);
			expect(isLegacyApplicationRoot('/Meetings')).toBe(true);
			expect(isLegacyApplicationRoot('/Standups')).toBe(true);
			expect(isLegacyApplicationRoot('/Decisions')).toBe(true);
			expect(isLegacyApplicationRoot('/Kanban')).toBe(true);
			expect(isLegacyApplicationRoot('/Brainstorming')).toBe(true);
			expect(isLegacyApplicationRoot('/Shares')).toBe(true);
		});

		it('returns false for workspace paths', () => {
			expect(isLegacyApplicationRoot('/Workspace/Notes')).toBe(false);
			expect(isLegacyApplicationRoot('/Workspace')).toBe(false);
		});

		it('returns false for nested or unknown paths', () => {
			expect(isLegacyApplicationRoot('/Notes/Subfolder')).toBe(false);
			expect(isLegacyApplicationRoot('/Random')).toBe(false);
			expect(isLegacyApplicationRoot('/')).toBe(false);
		});
	});
	...
	describe('getApplicationPathVariants', () => {
		it('returns both workspace and legacy paths', () => {
			expect(getApplicationPathVariants('Notes')).toEqual({
	...
	describe('legacy root policy compliance', () => {
		it('read paths include legacy roots so old data remains visible', () => {
			...
			for (const module of legacyRoots) {
				const paths = getApplicationReadPaths(module);
				expect(paths).toContain(getLegacyApplicationRoot(module));
				expect(paths).toContain(getApplicationRoot(module));
			}
		});
		...
		it('canonical write path is always under /Workspace', () => {
			...
			for (const module of allModules) {
				...
				expect(writePath).not.toBe(getLegacyApplicationRoot(module));
			}
		});
