...
// and forwards every signed kind-1/kind-9 event to Elembra's observation
// webhook, carrying the shared webhook HMAC. Elembra's webhook endpoint stays
// the authoritative verifier: HMAC + replay window + Nostr id/Schnorr signature +
...
//                                alpha-channel; a kind-9 event's `h` tag wins)
...
async function deliver(event) {
	// Channel attribution is bridge-side. Kind-9 stream messages carry the
	// authoritative channel in the NIP-29 `h` tag (canonical wire format,
	// spec: "Canonical publish tags and kinds") — it wins over the `channel`
	// tag and the configured default. Kind-1 legacy notes keep the `channel`
	// tag / default attribution unchanged.
	const findTag = (name) =>
		Array.isArray(event.tags)
			...
			: undefined;
	const hTag = event.kind === 9 ? findTag('h') : undefined;
...
function connect() {
	...
	const subscribe = () => {
		...
		subscribed = true;
		const filter = { kinds: [1, 9] };
		if (since !== undefined) filter.since = since;
		try {
			socket.send(JSON.stringify(['REQ', reqId, filter]));
			console.log(`subscribed ${reqId} kinds=[1,9] since=${since ?? 'all'}`);
		} catch (error) {
			subscribed = false;
	...
	socket.onmessage = async (raw) => {
		try {
			...
			if (kind === 'AUTH' && typeof message[1] === 'string') {
				...
				if (event && typeof event === 'object' && (event.kind === 1 || event.kind === 9)) {
