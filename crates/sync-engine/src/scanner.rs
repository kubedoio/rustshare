...
pub fn scan_local_root(root_path: &Path) -> Result<Vec<FileScanResult>> {
        ...
        .filter_map(|e| e.ok())
    {
        ...
        let relative_path = match absolute_path.strip_prefix(&canonical_root) {
            Ok(p) => p.to_path_buf(),
            Err(_) => {
                warn!("Could not compute relative path for: {}", absolute_path.display());
                continue;
            }
        };
        ...
        let metadata = match entry.metadata() {
            Ok(m) => m,
            Err(e) => {
                warn!("Failed to get metadata for {}: {}", absolute_path.display(), e);
                continue;
            }
        };
...
pub fn detect_local_changes<F>(
    current_scan: &[FileScanResult],
    mut db_lookup: F,
) -> LocalChanges
where
